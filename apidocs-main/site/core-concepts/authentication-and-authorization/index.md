...
5. Run this command to generate the token: `./generate_token.sh ${ClientId} ${KeyId} sts_private_key.pem (Ex. ./generate_token.sh ft-Docs-gateway-tester 9602ac55-46f0-4296-aea6-a5bfdbd845b9 sts_private_key.pem)`
...
```bash
curl -X POST --location "http://localhost:8084/v1/docs" \  -H "Accept: application/json" \  -H "Content-Type: application/json" \  -H "Authorization: Bearer ACCESS_TOKEN_GOES_HERE" \  -d "{  \"countryCode\": \"PK\",  \"description\": \"My first Docs\",  \"externalReferenceId\": \"Docs-test\",  \"subsidiaryId\": 2  }"
```
